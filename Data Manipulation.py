...
for claim_no in range(1, len(start_group) + 1):
    for c in range(int(start_group[claim_no]), int(end_group[claim_no])):
        ...
        index_data_list.append([claim_no, c, dev_quarter, occ_quarter])
...
seed(1)
train_prop = 0.7
max_claimno = index_data['claim_no'].max()
max_sequence = range(1, max_claimno + 1)

train_id = sample(max_sequence, int(train_prop * len(max_sequence)))

train_index = index_data.loc[index_data['claim_no'].isin(train_id)]
test_index = index_data.loc[~index_data['claim_no'].isin(train_id)]

train_set = databoxes.loc[databoxes['claim_no'].isin(train_id)]
test_set = databoxes.loc[~databoxes['claim_no'].isin(train_id)]

# Splitting test set into test and validation sets
test_claimnos = test_set['claim_no'].unique()

val_id = sample(list(test_claimnos), int(0.5 * len(test_claimnos)))

val_index = test_index.loc[test_index['claim_no'].isin(val_id)]
test_index = test_index.loc[~test_index['claim_no'].isin(val_id)]

val_set = test_set.loc[test_set['claim_no'].isin(val_id)]
test_set = test_set.loc[~test_set['claim_no'].isin(val_id)]
...
v1_train_index = train_index.loc[:, ['index', 'claim_no', 'pred_time', 
                                     'dev_quarter', 'occ_quarter', 'target', 
